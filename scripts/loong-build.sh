...
build_package() {
    ...
    if [[ -f $LOONGREPO/$PKGBASE/PKGBUILD ]]; then
        ...
        if [[ -z "$PKGVER" ]]; then
            ...
            if [[ $TESTING == "-staging" ]]; then
                PKGVER=`$SCRIPTSPATH/compare86.py -sp $PKGBASE |grep x86_64|grep staging|awk -F= '{print $2}'`
            else
    ...
    if [[ "$BUILDER" == "localhost" ]]; then
        ...
        ssh -t $BUILDER "cd /home/arch/repos/$PKGBASE; PACKAGER=\"$PACKAGER\" extra$TESTING-loong64-build $CLEAN -- -- -A -L $EXTRAARG" 2>/dev/null
        EXITCODE=$?
